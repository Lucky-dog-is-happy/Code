...
void put(HashMapChaining *hashMap, int key, const char *val)
{
  ...
  while(cur) {
    if(cur->pair->key == key) {
      ...
      strcpy(cur->pair->val, val); // 若遇到指定key，则更新对应val并返回
      return;
    }
    cur = cur->next;
  }
  // 若无该key，则将该键值对添加到链表头部
...
/* 打印哈希表 */
void printHashMap(HashMapChaining *hashMap)
{
  for(int i=0; i<hashMap->capacity; i++) {
    Node *cur = hashMap->buckets[i];
    printf("[");
